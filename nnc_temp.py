...
def custom_loss(output, target, adjacency_matrix, lambda_matrix, y_list):
...
def train(model, dataloader, optimizer, lambda_matrix, device):
    model.train()
    running_loss = 0.0
...
def test(model, dataloader, lambda_matrix, device):
    model.eval()
    running_loss = 0.0
...
if __name__ == '__main__':
    ...
    lambda_matrix = 0.00001 * adjacency_matrix
    ...
    model = ButterflyNetwork(input_size, hidden_size, output_size, noise_std_dev).to(device)
    
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    if train:
        for epoch in tqdm(range(epochs)):
            train_loss = train(model, train_loader, optimizer, lambda_matrix, device)
            test_loss = test(model, test_loader, lambda_matrix, device)
